...
def get_gold_data():
    ...
    df1 = clean_yfinance(df1)
    df15 = clean_yfinance(df15)
    daily = clean_yfinance(daily)

    if (
        df1 is None or df1.empty
        or df15 is None or df15.empty
        or daily is None or daily.empty
    ):
...
def analyze_gold():
    ...
    try:
        ...
        if df1 is None:
            ...
            print("❌ ดึงข้อมูล GOLD ไม่สำเร็จ")
            return
        ...
        if (
            ...
            and should_send_signal(signal_type)

        ):

            send_line(message)

        # ถ้ากลับ NORMAL ให้ reset state
...
def webhook():
    ...
    # แสดง User ID ของคนที่ส่งข้อความเข้ามา
